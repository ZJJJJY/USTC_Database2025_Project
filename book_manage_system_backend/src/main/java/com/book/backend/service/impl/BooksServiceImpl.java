...
import com.book.backend.mapper.BookRatingMapper;
...
public class BooksServiceImpl extends ServiceImpl<BooksMapper, Books>
    ...
    public R<Page<Books>> searchBookPage(BasePage basePage) {
        ...
        List<Books> books = baseMapper.searchBookPageWithRating(pageInfo, condition, query);
        // 使用自定义 SQL 查询并按评分排序
        pageInfo.setRecords(books);
        return R.success(pageInfo, "获取图书信息成功");
    }
    ...
    @Override
    public R<String> rateBook(RatingDTO ratingDTO) {
        // 1. 校验数据
        if (ratingDTO.getScore() == null || ratingDTO.getScore() < 0 || ratingDTO.getScore() > 10) {
            return R.error("评分必须为 0~10 的整数");
        }

        // 2. 插入或更新评分
        BookRating rating = new BookRating();
        rating.setBookNumber(ratingDTO.getBookNumber());
        rating.setUsername(ratingDTO.getUsername());
        rating.setScore(ratingDTO.getScore());

        // 如果已存在评分则更新，否则插入新记录（根据业务逻辑可选）
        int rows = bookRatingMapper.insertOrUpdate(rating);

        if (rows <= 0) {
            return R.error("评分失败");
        }

        return R.success(null,"评分成功");
    }
